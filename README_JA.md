...
指定された HTTPS URL からテキストコンテンツを抽出し、サブLLMを使用して正確な要約を生成してトークン消費を節約します。
*注: 静的HTMLのみサポートされています。シングルページアプリケーション（SPA）は、ブラウザベースのレンダリングエンジンを組み込むことによるメンテナンスコストの増大と外部依存のリスクを避けるため、意図的にサポート対象外としています。SPAの内容を要約する必要がある場合は、メインのAIエージェントに直接処理させるか、手動でコンテンツをコピー＆ペーストしてください。*
