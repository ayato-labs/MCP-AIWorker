...
## Decision
We implement a lightweight, static syntax validation step in the `draft_code` pipeline that runs before the code is finalized/written to the filesystem. Additionally, we structure the error reporting from the Sub-LLM to Architect.
...
2.  **Automated Repair**:
    - If a syntax error is detected, the `draft_code` pipeline automatically constructs a repair prompt containing the error message and the original code, instructing the Sub-LLM to fix the syntax within a single retry attempt.
...
- **Complexity**: Slightly increases the complexity of the `draft_code` pipeline logic.
